...
This repository documents a randomised 576-response experiment and reproducible analysis of formal argument scaffolding on a single bias-sensitive LLM task. The prompt did not improve the prespecified strict-correctness outcome. Exploratory diagnostics show that it increased identification of the intended solution and reduced contradiction, while also increasing qualification and ambiguity. The experiment therefore demonstrates bounded causal changes in model-output behaviour and motivates, but does not confirm, a broader education and human-auditability research programme.

For a concise overview of the study, findings, and education-governance relevance, read the [project summary](PROJECT_SUMMARY.md). The [literature and policy context](docs/06_literature_and_policy_context.md) distinguishes what the experiment demonstrated from what related research and policy make plausible.
...
| Opaque review identifiers and withheld condition metadata | Human oversight and evaluation independence |
...
### 5. Metadata-blinded human verification

Each response was presented under an opaque review-row identifier. The reviewer-facing workbook omitted prompt condition and response ID. One reviewer completed the coding, and human codes were joined back to experimental records only after review through a private linkage file. The formalised template remained visible in the response text, however, so its structure could reveal the likely condition. The procedure is therefore best understood as metadata-blinded rather than fully condition-masked. The frozen protocol and public variable codebook are in [`design/`](design/).
...
The confirmatory outcome was fixed before collection. The exploratory contrasts were selected after the primary result was known, although the underlying response codes were collected during metadata-blinded review. Their role is hypothesis generation.
...
The formal prompt increased father-solution identification by 10.4 points and reduced contradiction by 11.1 points, while also increasing unresolved-possibility language by 16.3 points and ambiguity by 9.4 points. These are prompt effects within the fixed experimental setting, but the outcomes were selected for analysis after the primary result was known. All four contrasts were defined as one exploratory family and adjusted with the Holm method; they require confirmation on fresh data. No father-identifying response in either condition was coded contradictory, so the overall contradiction reduction should not be described as cleaner reasoning among responses that recognised the intended solution.
...
Related research makes the project's education-governance hypothesis plausible without turning it into a demonstrated outcome. Structured explanations can improve inspectability without proving that the displayed reasoning faithfully records the computation behind an answer. Critical-thinking transfer generally depends on sustained, domain-informed practice rather than a one-off template. Current EU policy creates a need for context-sensitive AI-literacy measures and effective human oversight, but this scaffold is a candidate audit practice rather than a compliance mechanism.

The full [literature and policy context](docs/06_literature_and_policy_context.md) explains these connections, the current scope of EU AI Act Articles 4 and 14, and the distinction between evidence from this experiment and hypotheses for later human studies.
...
The dataset contains complete model outputs. It also contains collection metadata, parser outputs, repair provenance, and human judgments. Column suffixes identify provenance: `_auto` means deterministic parser output, while `_human` means human coding performed without condition metadata or response IDs.
...
- [`docs/06_literature_and_policy_context.md`](docs/06_literature_and_policy_context.md), which relates the findings to prompting, education, and AI-governance evidence.
...
One reviewer completed the final accuracy coding. Prompt condition, response ID, and the private linkage were withheld, but the visible formal template could reveal the likely condition. The review was therefore metadata-blinded rather than fully condition-masked, and the study cannot estimate inter-rater reliability. A replication should use two independent reviewers and report agreement before adjudication.
